...
<body>
    ...
    <script>
        ...
        // Add some demo data for testing (remove this in production)
        setTimeout(() => {
            if (window.dashboard && !window.dashboard.isConnected) {
                ...
                setInterval(() => {
                    demoData.coPPM = Math.random() * 300 + 20;
                    demoData.soundDB = Math.random() * 80 + 25;
                    demoData.pressCount = Math.floor(Math.random() * 7);
                    window.dashboard.updateDashboard(demoData);
                }, 3000);
            }
        }, 2000);
