...
const int BUTTON = 18;
const int BUZZER = 19;
const int SOUND_SENSOR = 34;  // ADC1_CH6 (analog input only)
...
void loop() {
  ...
  // Handle Sound System (Button Logic)
  handleSoundSystem();
  
  // Update OLED Display (Cycle between CO and Sound data)
  updateOLEDDisplay();
...
void handleSoundSystem() {
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    if (reading == LOW && buttonState != LOW) {
      ...
      // Read analog sound sensor value and map to dB scale
      int soundValue = analogRead(SOUND_SENSOR);
      int soundDb = map(soundValue, 0, 4095, 30, 120);
      int soundDB = (soundDb * 2);
      ...
      if (soundDB >= 66) {
        ...
        SendSoundViolationSMS(soundDB, pressCount, "HIGH_DB");
      } else {
      ...
      if (pressCount > 5) {
        ...
        SendSoundViolationSMS(soundDB, pressCount, "COUNT_EXCEEDED");
        pressCount = 0;
      }
...
void SendSoundViolationSMS(int soundDB, int honkCount, String violationType) {
  ...
  // Calculate fine same as sound code logic
  int totalSoundFine = (highDbCount * 20) + (countExceedPenaltyCount * 50);
  
  // Create violation message with ACTUAL Sound reading
  String SMS = "SOUND VIOLATION\n";
  SMS += "dB:" + String(soundDB) + " (Limit:66)\n";
  SMS += "Honks:" + String(honkCount) + "/5\n";
  SMS += "Fine: Rs" + String(totalSoundFine) + "\n";
  ...
  Serial.println("Honk Count: " + String(honkCount));
  Serial.println("Fine Amount: Rs" + String(totalSoundFine));
  Serial.println("------------------------");
  ...
  SerialAT.println("AT+CMGS=\"" + number + "\"\r"); //Mobile phone number to send message
  delay(1000);
  SerialAT.println(SMS);  // Send violation message with actual Sound reading
  delay(100);
  SerialAT.println((char)26);// ASCII code of CTRL+Z
  delay(1000);
  _buffer = _readSerial();
