...
// Sends violation SMS for both CO2 and Sound violations
...
// WiFi credentials
const char* ssid = "motog54";
const char* password = "moto9090";
...
int _timeout;
String _buffer;
String number = "+919804888535";  // REPLACE WITH YOUR NUMBER
...
<head>
    ...
    <style>
        * {
        ...
        .control-icon { background: linear-gradient(45deg, #ff9a9e, #fecfef); }
        ...
        <div class="dashboard">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div>
                        <div class="card-title">CO2 Monitoring</div>
                        <div class="card-subtitle">Carbon Dioxide Levels</div>
                    </div>
                </div>
            ...
            <div class="card">
                ...
                <div class="metric">
                    <span class="metric-label">CO2 Violations:</span>
            ...
            <div class="card">
             ...
             <!-- NEW Control and Configuration Card -->
            <div class="card">
                <div class="card-header">
                    <div class="card-icon control-icon">🕹️</div>
                    <div>
                        <div class="card-title">Controls & Config</div>
                        <div class="card-subtitle">Remote Device Management</div>
                    </div>
                </div>
                <div>
                    <button class="control-button" onclick="sendControl('send_sms')">Send Test SMS</button>
                    <button class="control-button" onclick="sendControl('recalibrate')">Force Recalibration</button>
                    <button class="control-button control-button-danger" onclick="sendControl('reboot')">Reboot Device</button>
                </div>
                 <div style="margin-top:20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top:15px;">
                    <div class="metric-label">Registered SMS Number:</div>
                    <div id="currentPhoneNumber" style="font-weight:700; font-size: 1.2rem; margin-top:5px; margin-bottom:10px; color:#feca57;">--</div>
                    <div class="number-input-group">
                        <input type="text" id="phone_number_input" placeholder="New +91 number">
                        <button class="control-button" style="margin-bottom:0; width:80px;" onclick="updateNumber()">Save</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        class SonicGuardDashboard {
            ...
            updateDashboard(data) {
                ...
                document.getElementById('currentPhoneNumber').textContent = data.phoneNumber || '--';
            ...
            checkViolations(data) {
                ...
                if (data.coPPM > 1000) {
                    hasViolation = true;
                    message = `CO2 Violation: ${Math.round(data.coPPM)} mg/km detected!`;
                }
        ...
        function sendControl(command) {
            if (window.dashboard && window.dashboard.websocket) {
                ...
                window.dashboard.websocket.send(command);
            } else {
                console.error("WebSocket is not connected.");
            }
        }
        function updateNumber() {
            const newNumber = document.getElementById('phone_number_input').value;
            if (newNumber && newNumber.length > 10) { // Basic validation
                 sendControl('update_number:' + newNumber);
                 document.getElementById('phone_number_input').value = ''; // Clear input field
            } else {
...
void forceRecalibration();   // Function prototype

// Handle WebSocket events, including incoming commands from the dashboard
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  switch(type) {
    ...
    case WStype_CONNECTED:
      ...
      break;
    case WStype_TEXT:
      { // New scope for payloadString
        String payloadString = (const char*)payload;
        Serial.printf("[%u] Received Text: %s\n", num, payloadString.c_str());

        // Parse incoming commands from the web dashboard
        if (payloadString == "send_sms") {
          SendMessage();
        } else if (payloadString == "reboot") {
          ESP.restart();
        } else if (payloadString == "recalibrate") {
          forceRecalibration();
        } else if (payloadString.startsWith("update_number:")) {
          // Extract the new phone number from the message
          String newNumber = payloadString.substring(payloadString.indexOf(':') + 1);
          if (newNumber.length() > 10) { // Basic validation
             number = newNumber;
             Serial.println("Phone number updated to: " + number);
...
// Packages all sensor data into a JSON string and broadcasts it to all WebSocket clients
void sendWebSocketData() {
  ...
  jsonDoc["phoneNumber"] = number; // Add current number to JSON packet
...
// Function to trigger a new calibration cycle for the MQ-7 sensor
void forceRecalibration() {
  Serial.println("!!! FORCING RECALIBRATION via DASHBOARD !!!");
  isCalibrated = false;
  initializationComplete = false; // Put system back into init mode display
  calibrationStartTime = millis();
  currentInitStatus = "Recalibrating...";
}


// Main setup function: Initializes hardware and connects to network services
void setup() {
  Serial.begin(115200);
  delay(1000);
  ...
  // Start CO2 Sensor calibration
  Serial.println("=== STARTING MQ7 SENSOR CALIBRATION (15 seconds) ===");
  ...
  Serial.println("Automatic violation SMS when CO2 > 200 ppm OR Sound violations");
}
...
void loop() {
  ...
  // Read MQ7 Sensor, unless in a forced recalibration
  if (isCalibrated) {
    if (millis() - lastReadTime >= READ_INTERVAL) {
      readMQ7Sensor();
      lastReadTime = millis();
      ...
      // While recalibrating, keep calling the read function to update progress
      readMQ7Sensor();
  }
...
void readMQ7Sensor() {
  ...
  if (!isCalibrated) {
    if (millis() - calibrationStartTime >= CALIBRATION_TIME) {
      ...
      Serial.println("System ready for CO2 monitoring...");
      ...
      currentInitStatus = "CO2: " + String(remainingTime / 1000) + "s left";
    }
    return;
  }
...
void checkCOViolation(float coPPM) {
  if (coPPM > CO_THRESHOLD) {
    if (!inCOViolation) {
      ...
      Serial.println("🚨 Potential CO2 violation detected, checking stability...");
    } else {
      ...
      if (!coViolationStable && (millis() - coViolationStartTime >= 5000)) {
        ...
        Serial.println("🚨 CO2 VIOLATION CONFIRMED - Sending SMS!");
    ...
    if (inCOViolation) {
      Serial.println("✅ CO2 Violation cleared - CO2 levels back to normal");
      inCOViolation = false;
      coViolationStable = false;
    }
  }
...
// Send CO2 Violation SMS with UPDATED FINE STRUCTURE
void SendCOViolationSMS(float coPPM) {
  Serial.println("=== SENDING CO2 VIOLATION SMS ===");
  ...
  coViolationCounter++;
//Total 160 characters support for the gsm module
  // Create violation message with ACTUAL CO2 reading
  String SMS = "EMISSION VIOLATION\n";
  SMS += "CO2:" + String(coMgKm)+ " mg/km " + " (Limit:" + String(limitMgKm) + " mg/km)\n";  // SIMULATED mg/km HERE
  ...
  Serial.println("CO2 Message to send:");
  Serial.println(SMS);
  Serial.println("Length: " + String(SMS.length()) + " chars");
  Serial.println("Actual CO2 Level: " + String(coMgKm, 1) + " mg/km (internal ppm: " + String(coPPM, 1) + ")");
  ...
  Serial.println("CO2 Violation SMS sent to: " + number);
  Serial.println("=== CO2 SMS SENDING COMPLETE ===");
}
